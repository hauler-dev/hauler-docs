...
`hauler store serve fileserver` runs a simple HTTP file server backed by the file, chart, and directory artifacts in the content store.
...
> **Note:** The fileserver only exposes files, charts, and directories (served as browsable folders). To serve container images, use [`hauler store serve registry`](./registry.md). To serve git repositories for cloning, use [`hauler store serve git`](./git.md).
